...
// Recursive function to find the longest increasing path starting from (i, j)
int longestPathRecursive(const vector<vector<int>>& matrix, int i, int j, vector<vector<int>>& dp) {
    ...
    for (auto dir : dirs) {
        ...
        if (newRow >= 0 && newRow < matrix.size() && newCol >= 0 && newCol < matrix[0].size() && matrix[newRow][newCol] > matrix[i][j]) {
            maxLength = max(maxLength, 1 + longestPathRecursive(matrix, newRow, newCol, dp));
        }
    }
...
int main() {
    ...
    for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < cols; ++j) {
            result = max(result, longestPathRecursive(matrix, i, j, dp));
        }
    }
